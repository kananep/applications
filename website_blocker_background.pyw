...
host_temp = r'C:\Users\toy\Desktop\appdevelopment\websiteblocker\hosts'
...
while True:
    if dt(dt.now().year,dt.now().month,dt.now().day,16) < dt.now() < dt(dt.now().year,dt.now().month,dt.now().day,21):
        print('Working Hours..........')
